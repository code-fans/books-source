...
double Limit(coordinate a,int d)            //找出所给坐标对应的圆心允许范围
{
  return sqrt(d*d-a.y*a.y);                 //用到勾股定理
}
...
int main()
{
  ...
  for(int Case=1; cin>>n>>d,n; Case++)      //n为0则退出
  {
    ...
    {
      cin>>p[k].x>>p[k].y;
      if(abs(p[k].y)>d)                     //坐标垂直距离超过d,无解
        Ok=0;                               //不能直接退出，否则数据会乱
      b[k].a=p[k].x-Limit(p[k],d);          //计算点在x轴的左边界
      b[k].b=p[k].x+Limit(p[k],d);          //计算点在x轴的右边界
    }
    if(Ok==0)
